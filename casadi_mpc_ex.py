...
def main():
    ...
    p_x, p_y, yaw, v_x, v_y, omega, theta = (
        X[0, :],
        X[1, :],
        X[2, :],
        X[3, :],
    ...
    def dynamics(x, u):
        ...
        dxdt = ca.vertcat(
            x[3] * ca.cos(x[2])
            - x[4] * ca.sin(x[2]),  # dx/dt = vx * cos(yaw) - vy * sin(yaw)
            x[3] * ca.sin(x[2])
            + x[4] * ca.cos(x[2]),  # dy/dt = vx * sin(yaw) + vy * cos(yaw)
            x[5],  # dyaw/dt = omega
    ...
    opti.subject_to(v_x[0] == 0.1)  # 초기 속도
    ...
    plt.subplot(2, 1, 1)
    plt.plot(sol.value(yaw), label="yaw")
    plt.plot(sol.value(v_x), label="v_x")
    plt.plot(sol.value(v_y), label="v_y")
